...
import com.training.cats.CatsHomework.MonadTransformers.Response.Ok
...
object CatsHomework {
  object MonoidUsage {
    ...
    object ClientReport {
      ...
      implicit def clientReportSemigroup: Semigroup[ClientReport] = new Semigroup[ClientReport] {
        override def combine(x: ClientReport, y: ClientReport): ClientReport =
          new ClientReport(total = x.total.combine(y.total), lastProblemId = Semigroup.last[UUID].combine(x.lastProblemId, y.lastProblemId), kinds = x.kinds.combine(y.kinds), totalPerKind = x.totalPerKind.combine(y.totalPerKind))
      }
    }
    ...
    def aggregate(input: Iterable[Problem]): Map[Client, ClientReport] = {
      Monoid.combineAll {
        for {
          problem <- input
        } yield Map(problem.client -> new ClientReport(1, problem.id, NonEmptySet.of(problem.kind), Map(problem.kind -> 1)))
      }
    }
  }
  ...
  object PolyValidation {
    ...
    /** Payment card
      *
      * @param number
      *   Card number, must be 16 characters, only digits
      * @param expirationDate
      *   Expiration date, not validated
      * @param name
      *   Cardholder name, only upper/lower-case latin characters and spaces, not empty
      * @param securityCode
    ...
    def validate[F[_]](number: String, expirationDate: YearMonth, name: String, securityCode: String)(implicit
      ...
      val validNumber = if (number.matches("^\\d{16}$")) number.pure else NonEmptyList.one(InvalidNumber).raiseError
      val validSecurityCode = if (securityCode.matches("^\\d{3}$")) securityCode.pure else NonEmptyList.one(InvalidSecurityCode).raiseError
      val validName = if (name.matches("^[a-zA-Z ]*$") && name.trim.nonEmpty) name.pure else NonEmptyList.one(InvalidName).raiseError
      val validExpirationDate = expirationDate.pure

      (validNumber, validExpirationDate, validName, validSecurityCode).mapN(PaymentCard)
    }
  }
  ...
  object MonadTransformers {
    ...
    def parseRequest(raw: RawRequest): Option[Request] = {
      raw.op match {
        case "write"      => (raw.key, raw.value).mapN(Request.Write)
        case "read"       => raw.key.map(Request.Read)
        case "cas"        => (raw.key, raw.previousValue ,raw.value).mapN(Request.CompareAndSet)
        case "read-unref" => raw.key.map(Request.ReadUnref)
        case _            => None
      }
    }
    ...
    def executeRequest[F[_]: Monad](db: KVDatabase[F], request: Request): OptionT[F, Response] = {
      request match {
        case Request.Read(key) =>
          for {
            value <- OptionT(db.read(key))
          } yield Response.Found(value)
        case Request.Write(key, value) =>
          OptionT(db.write(key, value).map(_ => Option[Response](Ok)))
        case Request.CompareAndSet(key, expected, newValue) =>
          for {
            value <- OptionT(db.read(key))
            if value == expected
            ok <- executeRequest(db, Request.Write(key, newValue))
          } yield ok
        case Request.ReadUnref(key) =>
          for {
            value <- OptionT(db.read(key))
            found <- executeRequest(db, Request.Read(value))
          } yield found
      }
    }
    ...
    def runReq[F[_]: Monad](db: KVDatabase[F], raw: RawRequest): F[Option[Response]] = {
      val option = for {
        ...
        resp = executeRequest(db, request)
      } yield resp.value
      option.get
    }
  }
